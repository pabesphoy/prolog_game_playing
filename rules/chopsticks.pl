...
legal(jugador1, revive) :- true(control(jugador1)), 
                                (
                                    (\+alive(rightj1), 
                                    alive(leftj1), 
                                    true(handup(jugador1, leftj1, F2)), 
                                    ((F2 = 4) ; F2 = 2))
                                    ;
                                    (alive(rightj1), 
                                    \+alive(leftj1), 
                                    true(handup(jugador1, rightj1, F1)), 
                                    ((F1 = 4) ; F1 = 2))
                                ).
legal(jugador2, revive) :- true(control(jugador2)), 
                                (
                                    (\+alive(rightj2), 
                                    alive(leftj2), 
                                    true(handup(jugador2, leftj2, F2)), 
                                    ((F2 = 4) ; F2 = 2))
                                    ;
                                    (alive(rightj2), 
                                    \+alive(leftj2), 
                                    true(handup(jugador2, rightj2, F1)), 
                                    ((F1 = 4) ; F1 = 2))
                                ).
...
next(handup(R, H, NewFingers)) :-
    does(R2, hit(Hitterhand, H)),
    true(handup(R, H, Fingers)),
    true(handup(R2, Hitterhand, HitFingers)),
    NewFingers is Fingers + HitFingers, 
    NewFingers < 5.
next(handup(R, H, 0)) :-
    does(R2, hit(Hitterhand, H)),
    true(handup(R, H, Fingers)),
